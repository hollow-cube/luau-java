...
public class TestLuaError {
    ...
    void stripDefaultErrorText() {
        var input =
            "[string \"test.luau\"]:1: invalid argument #1 to 'errfunc' (MyType expected, got number)";
        var result = LuaStateImpl.stripDefaultErrorPrefix(input);

        assertEquals(
            "invalid argument #1 to 'errfunc' (MyType expected, got number)",
            result
        );
    }
    ...
    void typeErrorShouldThrowLuaError2(LuaState state) {
        ...
        assertEquals(
            "invalid argument #-1 (MyType expected, got string)",
            err.getMessage()
        );
    }
    ...
    void checkStackThrowLuaError(LuaState state) {
        var err = assertThrows(LuaError.class, () ->
            state.checkStack(Integer.MAX_VALUE, "test error")
        );
    ...
    void checkStackThrowLuaError2(LuaState state) {
        var err = assertThrows(LuaError.class, () ->
            state.checkStack(Integer.MAX_VALUE, null)
        );
    ...
    @Test
    void throwDuringResume(LuaState state, Arena arena) {
        state.openLibs();
        load(
            state,
            """
        ...
        var err = assertThrows(LuaError.class, () -> state.resume(null, 0));
        assertMatches(
            """
                net.hollowcube.luau.LuaError: test error
                	at lua.<anonymous>(test.luau:1)
                	at net.hollowcube.luau.LuaStateImpl.resume(LuaStateImpl.java:0)
                	at net.hollowcube.luau.TestLuaError.lambda$throwDuringResume$0(TestLuaError.java:0)
                """,
            printStackTrace(err, 4)
        );
    }
