...
record LuaFuncImpl(
    ...
    LuaFuncImpl(ToIntFunction<LuaState> impl, String debugName, @Nullable Arena arena) {
        Arena actualArena = Objects.requireNonNullElseGet(arena, Arena::ofShared);
        final MemorySegment funcRef = lua_CFunction.allocate(new CFunctionWrapper(impl), actualArena);
    ...
    @Override
    public void close() {
        Objects.requireNonNull(this.closeableArena, "LuaFuncs allocated in provided arena may not be closed.")
                .close();
    }

    record CFunctionWrapper(ToIntFunction<LuaState> impl) implements lua_CFunction.Function {
        @Override
        public int apply(MemorySegment L) {
            ...
            try {
                ...
                if (err.status() == LuaStatus.ERRMEM)
                    return -100 - LuaStatus.ERRMEM.id();
                ...
                if (t instanceof Error e) {
                    System.err.println("An unrecoverable error occurred within a LuaFunc, the VM will crash.");
                    throw e;
                }
